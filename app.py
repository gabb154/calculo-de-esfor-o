...
def create_ui():
    ...
    # Controlando o número de postes e direções
    all_postes_data = []
    ...
    with st.form("form_projeto"):
        for i in range(st.session_state.num_postes):
            with st.expander(f"Dados para o Poste #{i+1}", expanded=True):
                ...
                # Adicionando número de direções de forma dinâmica
                num_direcoes = st.number_input("Número de direções de esforço:", min_value=1, max_value=6, value=1, step=1, key=f"num_dir_{i}")
                direcoes = []
                tem_compacta_poste = False

                # Gerar os campos de direção conforme a quantidade selecionada
